...
const {
  RegisterSchema,
  LoginSchema,
} = require("../../../middleware/v1/validations");
...
const SignUp = async (req, res) => {
  try {
    ...
    if (val.error) {
      return res
        .status(404)
        .json({ message: "data filled not appropriate", status: 404 });
    }
    ...
    if (data.flag === 1) {
      return res
        .status(301)
        .json({ info: "Duplicate email or number", status: 301 });
    }
...
const SignIn = async (req, res) => {
  try {
    ...
    if (val.error) {
      return res
        .status(404)
        .json({ info: "data filled not appropriate", status: 404 });
    }
